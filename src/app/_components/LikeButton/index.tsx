...
export function LikeButton({ photoId, data }: { photoId: string, data: Like }) {
    ...
    <div className={styles.field}>
      <div>
        <button onClick={handleLike} disabled={isProcessing} className={clsx(
          styles.like__button__common,
          like?.liked ? styles.like__button__liked : styles.like__button__normal)}>
          <span className={clsx(
            styles.icon__common, 
            styles.icon__img, 
            like?.liked ? styles.icon__color_liked : styles.icon__color__normal)} />
          <span className={styles.span__likes}>{`${like?.likes}`}</span>
        </button>
      </div>
    </div>
  );
